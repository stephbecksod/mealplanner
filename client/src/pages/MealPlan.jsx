...
const MealPlan = () => {
  ...
  const handleCustomMealSubmit = async (recipe, servings) => {
    setAddingCustomMeal(true)
    try {
      await addMeal(recipe, servings)
      setShowCustomMealForm(false)
      setShowAddMealModal(false)
    } catch (error) {
      console.error('Failed to add custom meal:', error)
      showError('Failed to add custom meal. Please try again.')
    } finally {
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-gray-800">
                {showCustomMealForm ? 'Add Custom Recipe' : 'Add Meal'}
              </h3>
              <button
                onClick={() => {
                  setShowAddMealModal(false)
              ...
              <>
                ...
                <div className="mb-6">
                  <button
                    ...
                    className="w-full p-4 border-2 border-gray-300 rounded-lg transition-colors flex items-center justify-center gap-3 hover:border-gray-400 hover:bg-gray-50"
                  >
                    <svg className="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                    <span className="font-semibold text-gray-700">Add Custom Recipe</span>
                  </button>
                  <p className="text-sm text-gray-500 text-center mt-2">
                    Enter your own recipe manually
                  </p>
                </div>
